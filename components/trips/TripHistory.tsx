'use client';

import { useState } from 'react';
import { useTrips } from '@/hooks/useTrips';
import MarkdownRenderer from '@/components/MarkdownRenderer';
import type { Trip } from '@/types';

interface TripHistoryProps {
  showToast: (msg: string, type?: 'success' | '') => void;
}

const DEST_EMOJIS: Record<string, string> = {
  goa: '🏖️', rajasthan: '🏜️', kerala: '🌴', manali: '🏔️', ladakh: '❄️',
  varanasi: '🕌', andaman: '🌊', coorg: '☕', ooty: '🌸', rishikesh: '🏕️',
  jaipur: '🏯', mysuru: '👑', hampi: '⛩️', shimla: '🎿', darjeeling: '🍃',
  default: '🗺️',
};

function getEmoji(destination: string) {
  const key = destination?.toLowerCase().split(' ')[0] || 'default';
  return DEST_EMOJIS[key] || DEST_EMOJIS.default;
}

export default function TripHistory({ showToast }: TripHistoryProps) {
  const { trips, deleteTrip } = useTrips();
  const [expandedId, setExpandedId] = useState<string | null>(null);

  const handleDelete = (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (expandedId === id) setExpandedId(null);
    deleteTrip(id);
    showToast('Trip deleted');
  };

  const markdownToPlain = (md: string): string =>
    md
      .replace(/^#{1,6}\s+(.+)$/gm, (_, t) => `\n${t.toUpperCase()}\n${'─'.repeat(t.length)}`)
      .replace(/\*\*(.+?)\*\*/g, '$1')
      .replace(/\*(.+?)\*/g, '$1')
      .replace(/__(.+?)__/g, '$1')
      .replace(/_(.+?)_/g, '$1')
      .replace(/\[([^\]]+)\]\([^)]+\)/g, '$1')   // [text](url) → text
      .replace(/`{1,3}[^`]*`{1,3}/g, '')          // strip code
      .replace(/^[-*•]\s+/gm, '• ')               // normalise bullets
      .replace(/^\d+\.\s+/gm, (m) => m)           // keep numbered lists
      .replace(/^---+$/gm, '─────────────────')   // hr
      .replace(/\n{3,}/g, '\n\n')                  // collapse blank lines
      .trim();

  const handleCopy = async (trip: Trip, e: React.MouseEvent) => {
    e.stopPropagation();
    const text = `${trip.name}\n${'─'.repeat(40)}\n\n${markdownToPlain(trip.result)}`;
    await navigator.clipboard.writeText(text);
    showToast('✅ Itinerary copied to clipboard!', 'success');
  };

  const handleShare = async (trip: Trip, e: React.MouseEvent) => {
    e.stopPropagation();
    const plain = markdownToPlain(trip.result);
    const fullText = `${trip.name}\n${'─'.repeat(48)}\n\n${plain}\n\n─────────────────────────────────────────────────\nPlanned with Roamai — roamai.in`;
    if (navigator.share) {
      try { await navigator.share({ title: trip.name, text: fullText }); } catch { /* user cancelled */ }
    } else {
      await navigator.clipboard.writeText(fullText);
      showToast('✅ Itinerary copied — paste into WhatsApp, email or notes!', 'success');
    }
  };

  const handlePrint = (trip: Trip, e: React.MouseEvent) => {
    e.stopPropagation();
    const win = window.open('', '_blank');
    if (!win) return;
    win.document.write(`<!DOCTYPE html><html><head><title>${trip.name}</title>
      <style>body{font-family:Georgia,serif;max-width:800px;margin:40px auto;padding:0 20px;color:#1a1a2e;line-height:1.7}
      h1{color:#8B1A1A}h2{color:#1a1a2e;margin-top:32px}strong{color:#333}
      .meta{color:#666;font-size:0.9rem;margin-bottom:32px;padding-bottom:16px;border-bottom:2px solid #eee}
      @media print{body{margin:20px}}</style></head>
      <body><h1>${trip.name}</h1>
      <div class="meta">📅 ${trip.days} days${trip.month ? ' · ' + trip.month : ''}${trip.people ? ' · ' + trip.people + ' people' : ''} · Generated by Roamai (roamai.in)</div>
      <pre style="white-space:pre-wrap;font-family:Georgia,serif">${trip.result.replace(/</g,'&lt;')}</pre>
      <script>window.onload=()=>{window.print();}</script></body></html>`);
    win.document.close();
  };

  if (trips.length === 0) {
    return (
      <div className="empty-state">
        <div className="empty-icon">🗺️</div>
        <div className="empty-title">No saved trips yet</div>
        <div className="empty-sub">Generate an itinerary in the AI Trip Planner — it will automatically appear here</div>
      </div>
    );
  }

  const expandedTrip = trips.find(t => t.id === expandedId);

  return (
    <div>
      <div className="trips-grid">
        {trips.map(trip => {
          const isOpen = expandedId === trip.id;
          const emoji = getEmoji(trip.destination);
          return (
            <div
              key={trip.id}
              className="trip-card"
              style={isOpen ? { border: '1.5px solid var(--orange)', boxShadow: '0 4px 20px rgba(249,115,22,.15)' } : {}}
              onClick={() => setExpandedId(isOpen ? null : trip.id)}
            >
              <div className="trip-card-banner">
                <span className="trip-card-emoji">{emoji}</span>
                <div>
                  <div className="trip-card-title">{trip.destination}</div>
                  <div className="trip-card-meta">
                    {trip.from ? `${trip.from} → ` : ''}{trip.destination}
                  </div>
                </div>
              </div>
              <div className="trip-card-body">
                <div className="trip-card-detail">
                  📅 {trip.days} {trip.days === 1 ? 'day' : 'days'}
                  {trip.month ? ` · ${trip.month}` : ''}
                  {trip.people ? ` · ${trip.people} ${trip.people === 1 ? 'person' : 'people'}` : ''}
                </div>
                <div style={{ fontWeight: 700, fontSize: '0.88rem', color: 'var(--text)', marginBottom: '10px' }}>
                  {trip.name}
                </div>
                {(trip.womenFriendly || trip.flags) && (
                  <div className="trip-flags">
                    {trip.womenFriendly && <span className="trip-flag flag-women">👩 Women Friendly</span>}
                    {trip.flags && trip.flags.split(', ').filter(f => f && f !== 'women-friendly').map(f => (
                      <span key={f} className="trip-flag">{f}</span>
                    ))}
                  </div>
                )}
              </div>
              <div className="trip-card-footer">
                <span style={{ fontSize: '0.78rem', fontWeight: 600, color: isOpen ? 'var(--orange)' : 'var(--muted)' }}>
                  {isOpen ? '▲ Close itinerary' : '▼ View full itinerary'}
                </span>
                <button className="btn danger sm" onClick={e => handleDelete(trip.id, e)} style={{ fontSize: '0.75rem' }}>
                  🗑️ Delete
                </button>
              </div>
            </div>
          );
        })}
      </div>

      {expandedTrip && (
        <div className="trip-expand">
          <div className="trip-expand-header">
            <div className="trip-expand-title">
              <span>{getEmoji(expandedTrip.destination)}</span>
              <span>{expandedTrip.name}</span>
            </div>
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              <button className="btn ghost sm" onClick={e => handleCopy(expandedTrip, e)} title="Copy to clipboard">📋 Copy</button>
              <button className="btn ghost sm" onClick={e => handleShare(expandedTrip, e)} title="Send full itinerary via WhatsApp, email or notes">📤 Send</button>
              <button className="btn ghost sm" onClick={e => handlePrint(expandedTrip, e)} title="Print / Save as PDF">🖨️ Print</button>
              <button className="btn ghost sm" onClick={() => setExpandedId(null)}>✕ Close</button>
            </div>
          </div>
          <div className="trip-expand-body">
            <MarkdownRenderer content={expandedTrip.result} />
          </div>
        </div>
      )}
    </div>
  );
}
